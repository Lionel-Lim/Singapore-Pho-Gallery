...
from meal_plan import generate_weekly_meal_plan, get_meal_by_id
from posts import (
    create_post,
    get_post_with_recipe,
    get_all_posts,
...
def api_create_user(user: UserCreateRequest):
    return create_user(
        user.name, user.email, user.diet, user.excluded_ingredients, user.cuisine
    )
...
@app.get("/get-meal-by-id/{meal_id}")
def api_get_meal_by_id(meal_id: str):
